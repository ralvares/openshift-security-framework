...
The network must enforce boundaries. By default, OpenShift allows unrestricted traffic between all pods. NetworkPolicy is the primary tool to restrict this, allowing teams to implement a "default deny" posture where only necessary connections are permitted. To ensure consistent governance across the cluster, AdminNetworkPolicy allows security teams to enforce mandatory rules that developers cannot override, while BaselineAdminNetworkPolicy provides a default fallback for any traffic not explicitly handled. Segmentation becomes explicit, documented, and testable—exactly what compliance frameworks expect when assessing lateral-movement risk.
