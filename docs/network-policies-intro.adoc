...
NetworkPolicies control which pods can talk to which pods, and which external networks they’re allowed to reach. They operate at the Kubernetes API/network-plugin layer using labels and selectors; they do not replace node-level or cloud firewall rules, but they are the native, in-cluster mechanism for segmentation.

If you don’t define any NetworkPolicies, your cluster behaves like the "everything talks to everything" house: pods can freely connect to each other and to external IPs unless other layers restrict them. Modern clusters need segmentation; NetworkPolicies are the native way to build it.

=== 2. How to Write Network Policies
...
=== 4. How NetworkPolicies Work (The Rules)
...
=== 6. Modern Additions and Operational Notes
